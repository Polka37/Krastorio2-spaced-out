...
for _, original_recipe in pairs(data.raw.recipe) do
	if original_recipe.categories then
		if original_recipe.categories[1] == data.raw.recipe["kr-singularity-tech-card"].categories[1] then
			...
			recipe.categories = { "kr-tech-cards-cooling" }
			...
			data_util.add_or_replace_ingredient(
				recipe.name,
				"fluoroketone-cold",
				{ type = "fluid", name = "fluoroketone-cold", amount = 10, fluidbox_index = 1 }
			)
			data_util.add_or_replace_ingredient(
				recipe.name,
				"kr-blank-tech-card",
			...
			data_util.add_or_replace_product(
				...
				{ type = "item", name = original_recipe.name, amount = 6 }
			)
			data_util.add_or_replace_product(
				recipe.name,
				"fluoroketone-hot",
